...
def upload_timelog(socket, ticket, jobid, timelogger, reqid):
  logfile = LOGS_PATH + '/mxnet' + reqid
  f = open("/tmp/localLogs", "wb").write(str({'lambda': reqid,
             'started': timelogger.start,
             'timelog': timelogger.points}).encode('utf-8'))
  crail.put(socket, "/tmp/localLogs", logfile, ticket, jobid) 
  print "wrote timelog"
  return

def upload_net_bytes(socket, ticket, jobid, rxbytes_per_s, txbytes_per_s, cpu_util, timelogger, reqid):
  ...
  crail.put(socket, "/tmp/localByteStats", netstats, ticket, jobid)
...
def upload_output_to_crail(socket, ticket, jobid, fileName, out):
  try:
    ...
    crail.put(socket, '/tmp/data.txt', fileName, ticket, jobid)
...
def handler(event, context):
  ...
  crail.create_dir(socket, 'mxnet-results', ticket, jobid)
  ...
  #}
  out = labels
  upload_output_to_crail(socket, ticket, jobid, outputKey, out)
  end = now()
  timelogger.add_point("upload results")

  print('Time to upload to crail is: {:.4f} s'.format(end - start))
  ...
  upload_timelog(socket, ticket, jobid, timelogger, lambdaID) 
  upload_net_bytes(socket, ticket, jobid, rxbytes_per_s, txbytes_per_s, cpu_util, timelogger, lambdaID)
