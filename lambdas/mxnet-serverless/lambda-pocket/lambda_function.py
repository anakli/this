...
def download_input_from_pocket(p, jobid, fileName, localfile=LOCAL_IMG_PATH):
  obj = pocket.get(p, fileName, localfile, jobid)
  

def upload_output_to_pocket(p, jobid, fileName, out):
  try:
    f = open('/tmp/data.txt', 'wb').write(out)
    pocket.put(p, '/tmp/data.txt', fileName, jobid)
...
def handler(event, context):
  ...
  pocket.create_dir(p, 'mxnet-results', jobid)
  ...
  #}
  out = labels
  upload_output_to_pocket(p, jobid, outputKey, out)
  end = now()
  timelogger.add_point("upload results")

  print('Time to upload to pocket is: {:.4f} s'.format(end - start))
  ...
  upload_timelog(p, jobid, timelogger, lambdaID) 
  upload_net_bytes(p, jobid, rxbytes_per_s, txbytes_per_s, cpu_util, timelogger, lambdaID)
