...
def download_input_from_crail(socket, ticket, fileName, localfile, oplog):
  obj = crail.get(socket, fileName, localfile, ticket, oplog)
  

def upload_output_to_crail(socket, ticket, fileName, out, oplog):
  try:
    f = open('/tmp/data.txt', 'wb').write(out)
    crail.put(socket, '/tmp/data.txt', fileName, ticket, oplog)
...
def lambda_s3_batch_handler(event, context):
  ...
  start = now()
  download_input_from_crail(socket, ticket, inputKey, LOCAL_IMG_PATH, oplog)
  end = now()
  ...
  #}
  out = labels
  upload_output_to_crail(socket, ticket, outputKey, out, oplog)
  end = now()
  ...
  upload_timelog(socket, ticket, timelogger, lambdaID, oplog) 
  upload_net_bytes(socket, ticket, rxbytes_per_s, txbytes_per_s, cpu_util, timelogger, lambdaID, oplog)
