...
uploadFileCount = 0
def upload_output_to_crail(socket, ticket, bucketName, filePrefix, fileExt):
  print('Uploading files to Crail: {:s}/{:s}'.format(bucketName, filePrefix))
  ...
  logfile = '/end2end' 
  f = open("/tmp/log.txt", "wb").write(str({'ops': oplog}).encode('utf-8'))
  crail.put(socket, "/tmp/log.txt", logfile, ticket, oplog)
...
def invoke_decoder_lambda(bucketName, filePrefix, startFrame, batchSize, lambdaID):
  ...
  response = client.invoke(FunctionName='decoder-scanner-crail',
...
def wait_until_all_finished(socket, ticket, startFrame, numRows, batch, videoPrefix):
  fileLists = []
...
def start_mxnet_pipeline(socket, ticket, test_video_path='videos/example.mp4', 
  ...
  crail.create_dir(socket, UPLOAD_PREFIX, ticket)
  crail.create_dir(socket, uploadPrefix, ticket)
  crail.create_dir(socket, "video-lambda-logs", ticket)
  crail.create_dir(socket, "video-lambda-logs-mxnet", ticket)

  if load_to_disk == True:
    # Upload all .proto files
    start = now()
    fileCount, totalSize = upload_output_to_crail(socket, ticket,
    ...
    stop = now()
    delta = stop - start
    print('Upload to crail time: {:.4f} s'.format(delta))
    ...
    fileCount = wait_until_all_finished(socket, ticket, 0, num_rows, batch, videoPrefix)
...
def ensure_clean_state(socket, ticket, test_video_path, batch):
  ...
  videoPrefix = test_video_path.split(".")[-2].split("/")[-1]
  print('Cleaning Crailt: {}/{}/{}_{}_{}/'.format(DOWNLOAD_BUCKET, 
...
if __name__ == '__main__':
  ...
  ensure_clean_state(socket, ticket, test_video_path, batch)
  start = now()
  start_mxnet_pipeline(socket, ticket, test_video_path, out_dir, decode_batch, batch, load_to_disk)
  stop = now()
  delta = stop - start
