...
def ensure_clean_state(socket, ticket, videoPath, batch, bucketName, prefix, args):
  ...
  print('Cleaning Crail db: {}/{}/{}_{}_{}/'.format(bucketName, 
...
uploadFileCount = 0
def upload_output_to_crail(socket, ticket, bucketName, filePrefix, fileExt, args):
  ...
  crail.put(socket, DECODER_PATH, "FusedDecodeHist-static", ticket)
  ...
  print('Uploaded {:d} files to Crail [total={:d}KB]'.format(uploadFileCount, 
    totalSize >> 10))
...
# Wait until all output files appear in Crail, return # files
def wait_until_all_finished(socket, ticket, startFrame, numFrames, videoPrefix, args):
  batch = args.batch
...
def start_fuse_pipeline(socket, ticket, videoPath, args):
  global timelist
  ...
  stop = now()
  delta = stop - start
  print('Upload args to Crail time: {:.4f} s'.format(delta))
  ...
  ################################################
  fileCount = wait_until_all_finished(socket, ticket, 0, numFrames, videoPrefix, args)
...
def main(args):
  ...
  ###################################################
  ensure_clean_state(socket, ticket, videoPath, batch, outputBucket, outputPrefix, args)
