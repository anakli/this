...
def put(socket, src_filename, dst_filename, ticket, oplog):  
  '''
  ...
  oplog.append("%f PUT %s" % (time.time(), dst_filename))

  return data

 
def get(socket, src_filename, dst_filename, ticket, oplog):  
  '''
  ...
  socket.sendall(pkt) 
  data = socket.recv(RESPONSE_BYTES)
  oplog.append("%f GET %s" % (time.time(), src_filename))
